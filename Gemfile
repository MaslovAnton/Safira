source 'http://rubygems.org'

gem 'rails', '3.0.9'

# Bundle edge Rails instead:
# gem 'rails', :git => 'git://github.com/rails/rails.git'

gem 'mysql2', '~> 0.2.7'
gem 'spree', :git => 'git://github.com/spree/spree.git', :branch => '0-60-stable'
gem 'spree_i18n', :git => 'git://github.com/spree/spree_i18n.git'
gem 'synergy', '~> 0.60.0'
gem 'synergy_default_theme', :git => 'git://github.com/MaslovAnton/synergy_default_theme.git'
gem 'spree_reviews', :git => 'git://github.com/MaslovAnton/spree-reviews.git'
gem 'spree_compare_products', :git => 'git://github.com/romul/spree_compare_products.git'
gem 'contact_us', :git => 'git://github.com/MaslovAnton/spree_contact_us.git'
gem 'recaptcha'
gem 'formtastic'
gem 'dynamic_form'
gem 'haml'
gem 'sass'

gem 'spree_variant_options', :git => 'git://github.com/MaslovAnton/spree_variant_options.git', :branch => 'master+'
gem 'spree_recently_viewed', :git => 'git://github.com/MaslovAnton/spree-recently-viewed.git'
gem 'spree_social', :git => 'git://github.com/MaslovAnton/spree_social.git'
#gem 'spree_share', :git => 'git://github.com/spree/spree_share.git'
gem 'hosted_gateway', :git => 'git://github.com/MaslovAnton/spree-hosted-gateway.git'
gem 'spree_email_to_friend', :path => 'vendor/spree_email_to_friend'
#gem 'spree_wishlist', :git => 'git://github.com/greendog/spree_wishlist.git'
gem 'spree_wishlist', :path => 'vendor/spree_wishlist'
gem 'spree_store_credits', :git => 'git://github.com/pdamer/spree_store_credits.git', :branch => '0-60-x'
gem 'spree_related_products', :path => 'vendor/spree_related_products'
#gem 'spree_related_products', :git =>  'git://github.com/romul/spree_related_products.git'
gem 'spree_vendors', :path => 'vendor/spree_vendors'
gem 'spree_free_shipping', :git => 'git://github.com/MaslovAnton/spree_free_shipping.git'

# Use unicorn as the web server
# gem 'unicorn'

# Deploy with Capistrano
# gem 'capistrano'

group :production do
  gem 'unicorn', '~> 4.0.1', :require => nil
end

group :development do
  gem 'capistrano-deploy', '~> 0.1.1', :require => nil
end
# To use debugger (ruby-debug for Ruby 1.8.7+, ruby-debug19 for Ruby 1.9.2+)
# gem 'ruby-debug'
# gem 'ruby-debug19', :require => 'ruby-debug'

# Bundle the extra gems:
# gem 'bj'
# gem 'nokogiri'
# gem 'sqlite3-ruby', :require => 'sqlite3'
# gem 'aws-s3', :require => 'aws/s3'

# Bundle gems for the local environment. Make sure to
# put test-only gems in this group so their generators
# and rake tasks are available in development mode:
# group :development, :test do
#   gem 'webrat'
# end

